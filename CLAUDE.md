...
- **Persisted interfaces** (kv-store, cache, scheduling) honor `EDGE_KV_STORE_PATH` / `EDGE_CACHE_PATH` / `EDGE_SCHEDULING_PATH` env vars. Absent or invalid → ephemeral in-memory only. **KV / cache / scheduling are per-tenant by design** — two apps of the same tenant share one namespace (apps opt in to sharing; task IDs are UUIDs so scheduling cross-app collision is theoretical). **Filesystem preopens (`EDGE_FS_PATH`, issue #558) are per-app**: the guest's `/` is mounted at `{EDGE_FS_PATH}/{tenant_id}/{app_name}/`. Per-tenant would let app A corrupt app B's SQLite (miserable to debug), so preopens get a different default.
