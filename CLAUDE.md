...
| `src/store.rs` | `create_store` attaches a `StoreLimits` via a `Box::leak`-ed `Box<StoreLimits>` to produce a `'static` pointer satisfying wasmtime's `ResourceLimiter` callback lifetime. `StaticLimiter::limiter` takes `&mut self` (not `&self`) to ensure at most one `&'static mut` reference exists at a time — the former `&self` receiver was UB (issue #176). |
