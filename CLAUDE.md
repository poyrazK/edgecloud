...
- **Port pool exhaustion** in `edge-worker/src/supervisor.rs::start_app` returns `Err(anyhow::Error)` when `PortPool::acquire()` returns `None` (fixed in issue #641 / PR-open). `acquire()` already returns `Option<u16>`; the call sites match on it and `anyhow::bail!` with a structured `tracing::error!` (carrying `tenant_id`, `app_name`, `deployment_id`, `free_slots`). The WS branch releases the just-acquired HTTP port before bailing so it doesn't leak into a 60-second cooling-down limbo. The consume loop's existing nack-and-continue path handles the `Err`. Regression coverage: `start_app_returns_err_when_port_pool_exhausted` in `edge-worker/src/supervisor.rs` (test-only `PortPool::drain_available_into_cooldown` helper constructs the exhausted state).
