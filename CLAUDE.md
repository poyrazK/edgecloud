...
- **CLI retry classification lives on `ApiError` (issue #571).** `ApiError::is_retryable()` at `edge-cli/src/api/client.rs` is the single source of truth for "should the caller retry?" — `Transient { .. }` and `Rejected { 429 }` are retryable; every other `Rejected` is deterministic and must NOT retry. Future retry-aware CLI commands (`edge env set`, `edge traffic set`, `edge keys create`, …) should call `ApiError::is_retryable()` instead of hand-rolling 4xx/5xx classification. Wrap the error with `anyhow::Error::new(api_err).context("...")` (NOT plain `anyhow!("...")`) so the typed `ApiError` survives the chain and `is_retryable` can downcast it. The `commands::deploy::is_anyhow_retryable` helper in `edge-cli/src/commands/deploy.rs` is the canonical pattern: walk `e.chain()`, downcast to `ApiError` first (defer to `is_retryable`), then to `reqwest::Error` (`!is_builder()` → transient, `is_builder()` → deterministic). Any error in the chain that isn't one of those is treated as deterministic (don't retry).
