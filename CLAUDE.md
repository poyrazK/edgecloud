...
- **`SocketEgressPolicy` modes (issue #309).** `edge-runtime/src/socket_egress.rs::SocketEgressPolicy` is the worker-wide switch for `wasi:sockets` outbound TCP/UDP connect. Four variants — `BlockAll` (default; deny every connect-side call), `AllowList` (consult `EgressPolicy::check_address` with the existing hard-deny + per-tenant allowlist layers), `AllowAll` (operator opt-in, **off by default**), and `HostnamePinned` (consult `EgressPolicy::hostname_pinned_match` against the per-tenant `HostnamePinning` resolution cache). The worker reads `EDGE_EGRESS_SOCKET_MODE` (`block-all` / `allow-list` / `allow-all` / `hostname-pinned`) once at startup in `Config::from_env` and threads it into every `HandlerConfig`/`RuntimeState`. See the module-level `HostnamePinned mode (dormant)` section in `socket_egress.rs` — `HostnamePinned` is dormant until the upstream wasmtime-wasi patch in `docs/upstream-wasmtime-resolve-check.patch` merges; until then it behaves as `BlockAll`.
- **`EDGE_EGRESS_HOSTNAME_PINNING` (dormant).** Optional bool env knob (parsed 1/0, true/false, yes/no, on/off, case-insensitive — default false). Mirrors `Config::hostname_pinning_enabled` and `HandlerConfig::hostname_pinning_enabled`. When `true`, the per-request RuntimeState swap uses `SocketEgressPolicy::HostnamePinned` instead of the worker-wide `Config::socket_mode`. Setting it to `true` today has no observable effect (the upstream resolve hook isn't wired — the cache stays empty, so `hostname_pinned_match` denies every connect-side call). Operator-facing knob for the dormant mode; flip after the upstream patch merges.
