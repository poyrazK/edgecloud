...
- **Metric families** — `edge_requests_total`, `edge_outbound_bytes_total`, `edge_resident_seconds_total`, `edge_duration_ms_total` (all labeled `{deployment_id, app_name}`); `edge_app_status{deployment_id, app_name, status}` (single-row invariant — previous status is cleared before a new one is stamped, see `metrics::WorkerMetrics::set_status`); `edge_worker_uptime_seconds`; `edge_worker_active_apps`.
