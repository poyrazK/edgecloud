...
- **Destructive CLI commands (issue #573).** `edge apps delete <name> --yes` is the first owner-role destructive subcommand (the CP's `RequireRole("owner")` middleware gates `DELETE /api/v1/admin/apps/{appName}`; the CLI loads a single bearer via `ApiKey::load` with no separate admin-token path, so a pre-flight `whoami` at `edge-cli/src/commands/apps.rs::check_owner_role` surfaces an actionable role-mismatch message instead of a generic 403). Convention for any future admin-only destructive command (`apps delete`, `tenant delete`, `secret reset`, etc.): (a) pre-flight `whoami` role check + multi-line actionable error on mismatch; (b) `--yes`/`-y` flag is REQUIRED (flat bail — no TTY confirm prompt, since the cascade reaches beyond the CP via `task_purge` outbox rows that tear down worker in-memory + on-disk state); (c) retry-aware via `call_with_retry_no_interrupt` with the default retry constants (DELETE is naturally idempotent — second call returns 404); (d) wrap typed `ApiError` via `.map_err(anyhow::Error::new)` so `is_anyhow_retryable` can downcast. The wire layer is `ApiClient::delete_app` at `edge-cli/src/api/client.rs:1147` (mirrors `Keys::revoke`). The cascade is irreversible (artifact blobs, env rows, active deployments, per-app KV/cache/scheduler dirs on every worker via `task_purge` per issue #569 / PR #601).
