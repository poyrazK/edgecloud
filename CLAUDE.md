...
| `edge-ingress/` | Rust | Public-facing Caddy controller. Subscribes `edgecloud.heartbeats.<region>`, renders Caddyfile-JSON that maps `<tenant>-<app>.edgecloud.dev` to a worker host:port. DDoS caps + per-IP rate limit. PRs #665 A/B/D: opt-in cross-replica `GLOBAL_RATE_LIMIT_RPS` aggregation (issue #665) — UDS SOCK_DGRAM reader task + Caddy splice between tenant_rl and per-app routes. |
| `edge-ingress-sidecar/` | Rust | Co-located with Caddy in each ingress pod. Scrapes Caddy's `/metrics`, publishes per-replica RPS deltas to a JetStream stream, consumes the platform total, computes a per-replica cap, and writes a `DatagramPayload` to a UDS SOCK_DGRAM socket once per tick (issue #665 PR B). The ingress binary (`edge-ingress`) is the UDS receiver; without it the sidecar's writes are silently dropped (fail-closed). |
| `edge-cli/` | Rust | Developer CLI (`edge init \| build \| deploy \| dev \| activate \| env \| apps \| webhooks \| domains \| keys \| traffic \| egress \| whoami \| ...`). The package is `edge-cli` but the installed binary is named `edge` (`[[bin]] name = "edge"` in edge-cli/Cargo.toml). Persists local state to `.edge/state.json`. Reads `~/.config/edgecloud/config.toml` via `edge-config`. `edge init --lang=rust` (issue #576) scaffolds a FaaS-shaped `src/lib.rs` + `Cargo.toml` modeled on `samples/hello/`, plus a vendored `wit/` tree (embedded into the binary via `include_dir!` — `edge-cli/src/scaffold/wit.rs`) so the scaffolded project builds offline outside the monorepo. `edge apps delete <name> --yes` (issue #573) is the only owner-role destructive subcommand — see the "Destructive CLI commands" Convention below. |
...
7. **Heartbeat** — the worker publishes `HeartbeatMessage{app_status, request_count, outbound_bytes, resident_seconds, duration_ms_total, status_5xx_count}` to `edgecloud.heartbeats.<region>` every **30s** (whitepaper §5.6) so the control plane can bill and monitor. Each `AppStatus` carries a `dedupe_id` (issue #418) — a stable `(worker_id, deployment_id, 30s_bucket)` token the CP uses to skip re-applying the same delta on JetStream redelivery or reconcile replay. `edge-ingress` subscribes to the same subject to learn routing. The `dedupe_id` is stamped on `AppStatus` in the JSON body (not as a NATS header) so the token survives `edge-ingress` re-publishes to a downstream subscriber. The four metered dimensions follow a strict LR-vs-FaaS split: **LongRunning** apps contribute `resident_seconds` (per-app ticker) but stamp `0` for `duration_ms_total` and `null` for `request_count`/`outbound_bytes` semantics not relevant to the resident axis; **Handler (FaaS)** apps contribute `request_count`, `outbound_bytes`, and `duration_ms_total` (per-request wall-clock latency stamped in each of the four `handle_request` terminal arms — `Ok(Ok)`, `Ok(Err)`, `Err(_dropped)` exit_code!=0, `Err(_dropped)` exit_code==0) but stamp `0` for `resident_seconds`. `duration_ms_total` was added by issue #555; legacy workers deserialize to `0` (Rust `#[serde(default)]` + Go `omitempty`-aware fold via `DurationMsTotalOrZero()`). `status_5xx_count` (issue #84 ask 6/7, FaaS only — bumped in `dispatch::synthetic_500` for all three error terminal arms) was added by PR #725 (PR A); legacy workers serialize without the field and the CP's `Status5xxCountOrZero()` folds absent → `0`. **Heartbeat key shape (issue #290):** the worker's `apps` map is keyed by `"{app_name}:{deployment_id}"` (e.g. `"my-app:d_1"`), so a canary fan-out of the same `(tenant, app_name)` produces N distinct keys instead of overwriting. `edge-ingress` parses the key with `split_once(':')` (already wired at `edge-ingress/src/heartbeats.rs`) to recover both halves. Legacy workers emitting bare `app_name` keys still parse as `deployment_id: None` on the ingress — pure forward-compat. See `docs/canary.md` for the full end-to-end contract.
...
- `edgecloud.tasks.<region>` — control plane → workers (`TaskMessage{timestamp, tenant_id, apps}` — `TaskUpdate`, `FullSync`, or `TaskPurge` variants). `TaskPurge` carries `{tenant_id, app_name?, reason: app_deleted|tenant_offboarded}` and triggers `edge_runtime::purge_tenant` on receipt (issue #569); empty `app_name` = tenant-wide. Same wire subject, distinct `type` discriminator.
...
2. The CLI POSTs to one of two endpoints:
   ...
   - `POST /api/v1/migrate-tree` (tree) — either multipart parts + a `tree` JSON manifest (`{"files":[...]}`) + one `file` part per entry, **or** a single `tree` part with `Content-Type: application/zip`. Required form field: `app_name` (must match `^[a-z0-9][a-z0-9.\-_]{0,62}$` — issue #438 widened the regex to allow `.` and `_` for semver-ish names like `myapp.v2` or `app_v2`). 50 MiB body cap. Accepted extensions: `.c`/`.h` (C) or `.rs` (Rust), depending on `language`. Note that since `app_name` may contain `.` or `_`, the public hostname `<tenant_id>-<app_name>.edgecloud.dev` can be **deeper than two labels** under `edgecloud.dev` (e.g. `t_acme-myapp.v2.edgecloud.dev`); operators must provision `*.*.edgecloud.dev` DNS + a matching multi-label cert (loadable via `TLS_CERT_FILE_2` / `TLS_KEY_FILE_2` in `edge-ingress`) before dotted names are routable. Caddy's anchored `host_regexp` matchers still cover the host; per-route `tls.on_demand: {}` is the ACME fall-through for any host the multi-label cert does not match.
...
- **Rollback** — restores `last_good_deployment_id` (set by `005_add_last_good` migration); enqueues its `task_update` `outbox` row inside the same tx. Emits a `rollback` webhook on the manual path; emits `auto_rollback` (with `trigger: worker_crash`, `canary: "false"`) when called from `POST /api/internal/apps/{appName}/auto-rollback` (issue #84 ask 7, PR A #725 — `RollbackDeployment` gains an `isAutoTriggered bool` arg; the handler at `internal.go:602` passes `true`, the manual `/api/v1/apps/{appName}/rollback` handler passes `false`). The constant `domain.WebhookEventAutoRollback = "auto_rollback"` was previously declared-but-never-fired; PR A closes that gap. The payload carries `trigger: operator` on the manual path so subscribers can distinguish the two paths without subscribing to both events. PR B will branch on `app_traffic_splits` and set `canary: "true"` when the worker triggers a canary retirement (out of scope for PR A — the field is currently hard-coded `"false"`).
...
- **App / tenant deletion → `task_purge`** (issue #569, issue #60) — `AppService.Delete` and `TenantService.DeleteTenant` each enqueue a `task_purge` `outbox` row inside the same tx as the cascade deletes. Per-app deletion enqueues one row scoped to that app; per-tenant deletion enumerates the tenant's apps first and enqueues one row per app (per-app keeps the worker's `handle_purge` path uniform with the app-deletion handler and avoids forcing the worker to enumerate its in-memory `(tenant, app)` keys). `OutboxDrainer.processRow` (Commit 2 of #569) switches on `row.Kind` and routes `task_purge` rows to the new `Publisher.PublishPurge`. Forward-compatible with #475/#476 (durable CP-side `tenant_kv` tier) — when those ship, sibling `tenant_kv BatchDelete` rows land at the same call site. **Issue #60 atomicity:** `AppService.Delete` runs the parent `apps` delete (via `appRepo.AtomicDelete` — `DELETE … RETURNING true` — `false` → `ErrAppNotFound`), the four child deletes (`app_env`, `active_deployments`, `app_traffic_splits`, `deployments`), and the `task_purge` outbox insert inside one `repository.Transaction`. Child delete order is enforced (traffic splits before deployments because `app_traffic_splits.deployment_id` has no `ON DELETE CASCADE`). Deployment IDs are captured inside the tx via `deployRepo.ListByApp` so the post-commit artifact pass can target every `.wasm` AND `.cwasm` (via the new `ArtifactStore.DeleteFormat`). Post-commit artifact errors are aggregated with `errors.Join` and returned so the HTTP handler maps them to 500 — the DB tx has already committed, so a follow-up `edge apps delete` against the same app would be a no-op (no deployments to clean) and return 204. Regression coverage: `TestAppService_Delete_*` in `edge-control-plane/internal/service/app_test.go`; `TestAppHandler_Delete_ServiceErrorReturns500` in `edge-control-plane/internal/handler/app_test.go`; `apps_delete_returns_500_after_retries` in `edge-cli/tests/apps.rs`.
...
- `AuditGC.Run` — issue #574. Retention GC for `audit_logs`. Default `AUDIT_RETENTION=90d`, `AUDIT_GC_INTERVAL=1h`. Server-side `NOW() - make_interval(secs => $1)` cutoff (clock-skew safe); paginated 10k rows/batch, 1000 batches/sweep cap, ctx-cancellation short-circuit. Index path: `idx_audit_logs_created_at` (migration 031). Same shape as `LogGC`.
- `WebhookDeliveryGC.Run` — issue #574. Retention GC for `webhook_deliveries`. Default `WEBHOOK_DELIVERY_RETENTION=30d`, `WEBHOOK_DELIVERY_GC_INTERVAL=1h`. Index path: `idx_webhook_deliveries_created_at` (migration 031). Note: `webhook_id` FK to `webhooks` is `ON DELETE CASCADE`, but the retention sweep only touches the deliveries side.
- `AutoscaleEventGC.Run` — issue #574. Retention GC for `autoscale_events`. Default `AUTOSCALE_EVENT_RETENTION=14d`, `AUTOSCALE_EVENT_GC_INTERVAL=1h`. Index path: `idx_autoscale_events_created_at` (migration 031). The existing `(region, created_at DESC)` index remains for the `ListRecent(region=)` read path.
...
The issue #574 retention GC trio (`AuditGC`, `WebhookDeliveryGC`, `AutoscaleEventGC`) follows the same pattern with three more factory methods (`NewAuditGCSink`, `NewWebhookDeliveryGCSink`, `NewAutoscaleEventGCSink`) wired alongside the existing three.

32 label-free families are exposed on BOTH `/metrics` (unauthenticated, operator) AND `/api/v1/metrics` (per-tenant) — the per-tenant endpoint appends the global GC families after the per-tenant/app series so tenants can see fleet-wide GC health on their own page:
...
- **`edge_audit_log_gc_*`** (4 families, issue #574, `edge-control-plane/internal/service/audit_gc.go`): `ticks_total`, `rows_deleted_total`, `errors_total`, `last_tick_timestamp_seconds`. Same shape as `edge_log_gc_*` — alert `last_tick_timestamp_seconds` if older than `2 * AUDIT_GC_INTERVAL`.
- **`edge_webhook_delivery_gc_*`** (4 families, issue #574, `edge-control-plane/internal/service/webhook_delivery_gc.go`): same 4-family shape; alert `last_tick_timestamp_seconds` if older than `2 * WEBHOOK_DELIVERY_GC_INTERVAL`.
- **`edge_autoscale_event_gc_*`** (4 families, issue #574, `edge-control-plane/internal/service/autoscale_event_gc.go`): same 4-family shape; alert `last_tick_timestamp_seconds` if older than `2 * AUTOSCALE_EVENT_GC_INTERVAL`.
...
The `ArtifactStore` interface (`storage/artifact.go`) covers `Save`/`Open`/`Delete`/`SaveFormat`/`OpenFormat`/`DeleteFormat`. Path components (tenant ID, app name, deployment ID) are validated by `validatePathComponent` to reject `..`, `/`, NUL, and other traversal chars. `MaxArtifactSize` (100 MiB) is enforced on both the read side (`Open`) and the write side (`Save`); handlers additionally cap request bodies via `http.MaxBytesReader` (`internal/middleware/maxbody.go`). `DeleteFormat(ctx, tenantID, appName, deploymentID, format)` (issue #60) deletes a non-default format alongside `.wasm` — currently used to remove the `.cwasm` precompiled artifact written by `precompile.PrecompileCwasm`; the FS / S3 / remote backends all implement it (404 / `os.ErrNotExist` are treated as success to mirror `Delete`). Called by `AppService.Delete` after the DB tx commits.

## Conventions & Gotchas

- **First-deploy smoke test (issue #646).** `edge-control-plane/internal/integration/tenant_first_deploy_test.go` exercises a brand-new tenant → owner API key → free quota → `POST /api/v1/deploy/{appName}` → activation → outbox fan-out → running-app heartbeat → `POST /api/internal/tokens/tenant` going from **403 (cold boot) to 200 (after running heartbeat)**. The cold-boot 403 and the post-heartbeat 200 are both asserted, plus an intermediate 403 after a capacity-only heartbeat to pin the seam. Run locally with `cd edge-control-plane && go test -tags=integration -v -count=1 -run TestTenantFirstDeploy ./internal/integration/...`. The test exposes the seam tracked by issue #642 (Worker JWT cold-boot window); it does NOT fix it. Depends on a shared `*middleware.WorkerKeyCache` being wired between `NewInternalHandler` and the route-middleware `workerAuth` config in `edge-control-plane/internal/app/app.go` (the `workerKeyCache := middleware.NewWorkerKeyCache(workerRepo.GetPublicKey)` block above the `internalHandler := handler.NewInternalHandler(...)` call). Without the share, `resolvePerWorkerSigningKey` nil-panics on every mint after a worker has enrolled, so the post-heartbeat 200 assertion turns red.
  - The smoke test is also the **first end-to-end exerciser of `UpsertStatus` against real Postgres since migration `034_worker_status_capacity`** landed. Two latent defects surfaced on the way to green and were fixed in PR #721 (the smoke test's PR):
    - `workers.tenant_id` is NOT NULL FK to `tenants(id)` per migration `003_workers_tenant_id`. A fake worker row can't use `tenant_id='*'` (the wildcard is the legacy JWT-claim convention, not a tenant row). Seed the FK column with the freshly-bootstrapped tenant ID instead — see `firstDeployNewFakeWorker` at `edge-control-plane/internal/integration/tenant_first_deploy_test.go:637`.
    - `UpsertStatus` at `edge-control-plane/internal/repository/worker.go:336` raised `pq: inconsistent types deduced for parameter $3` because the placeholder appears in two distinct type-deducing contexts (`VALUES` for `last_report TIMESTAMPTZ` AND `CASE THEN` for `last_exhaustion_at TIMESTAMPTZ`). The fix is an explicit `$3::TIMESTAMPTZ` cast on the CASE branch — Postgres could not reconcile the two contexts during PREPARE.
  - Two test-side gotchas are documented in the test file as regression markers — keep them green:
    - `bootstrapLimiter` (issue #420 ingress 402 layer) wraps `POST /api/v1/tenants`. The smoke test issues ONE bootstrap per run; the limiter is per-IP with `max=10`, so the test is well under it. Don't blast multiple bootstraps from the same IP without sleeping.
    - The deploy service stamps `outbox.dedupe_key` as `tenantID:appName:<fresh-uuid>` per attempt (`edge-control-plane/internal/service/deployment.go:1677`). Tests that want to assert the post-activate outbox row must query by `(tenant_id, app_name)` + `ORDER BY created_at DESC LIMIT 1`, not by `dedupe_key`.
    - `cfg.JWT.WorkerTokenTTL` defaults to 0 in `app.New` (the production config layer in `internal/config/config.go:684` upgrades 0 → 15m). Tests calling `app.New` directly with a hand-rolled `config.Config` MUST set `JWT.WorkerTokenTTL` explicitly, otherwise `expires_at` lands on the same wall-clock second as the test's `now()` and `expires_at > now` is satisfied only by sub-second rounding.
...
- **Port pool exhaustion** in `edge-worker/src/supervisor.rs::start_app` returns `Err(anyhow::Error)` when `PortPool::acquire()` returns `None` (issue #641). `acquire()` returns `Option<u16>`; the call sites match on it and `anyhow::bail!` with a structured `tracing::error!` (carrying `tenant_id`, `app_name`, `deployment_id`, `free_slots`). The WS branch releases the just-acquired HTTP port before bailing so it doesn't leak into a 60-second cooling-down limbo. The consume loop's existing nack-and-continue path handles the `Err`. **Counter (issue #641):** the worker stamps `HeartbeatMessage.port_pool_exhausted_count: u64` (cumulative since process boot, resets on restart) on every accepted heartbeat, and the CP's `MetricsAggregator` emits it as `edgecloud_worker_port_pool_exhausted_total` on both `/metrics` (global) and `/api/v1/metrics` (per-tenant). The CP also persists the value on `worker_status.port_pool_exhausted_count` for the deploy-time 402 gate. **Pool size:** `PortPool` is pre-populated at `Config::port_pool_size` slots (env `EDGE_PORT_POOL_SIZE`, default 100, validated to [1, u16::MAX]); see `edge-worker/src/port_pool.rs::with_capacity` and `edge-worker/src/config.rs`. **Capacity gate (issue #641):** `DeploymentService.Deploy` Pre-check 6 calls `workerRepo.SumFreeSlotsByRegion(ctx, effectiveRegions)` (90s heartbeat-freshness window, partial index `idx_worker_status_region_free_slots` on `worker_status(region, free_slots) WHERE free_slots >= 0`); when every region sums to 0 the deploy returns 402 `PaymentRequiredError{Reason: "region_at_capacity"}`. The gate fails open on transient SUM errors so a brief DB hiccup doesn't lock out every tenant. Migration `032_worker_status_capacity` adds the `free_slots` / `cluster_headroom` / `port_pool_exhausted_count` / `last_exhaustion_at` columns plus the partial index. **Operator rollout order:** ship CP migration `032` FIRST and upgrade workers AFTER — until workers send `ClusterHeadroom.free_slots`, every region's `SumFreeSlotsByRegion` returns 0 and the gate rejects every deploy. Regression coverage: `start_app_returns_err_when_port_pool_exhausted` + `start_app_releases_raw_port_when_ws_port_acquire_fails` in `edge-worker/src/supervisor.rs`; `TestDeploy_RegionAtCapacity_Returns402` + 3 sibling deploy-gate tests in `edge-control-plane/internal/service/deployment_test.go`; `TestWorkerRepository_SumFreeSlotsByRegion_*` in `edge-control-plane/internal/repository/worker_test.go`.
- **App-task panic propagation (issue #45).** `stop_app`'s `try_into_panic()` arm logs the panic payload at `tracing::error!` (with `tenant_id` / `app_name` / `panic_payload` structured fields) and returns `Ok(())` instead of `panic::resume_unwind`. The panic-in-`run_app_loop` path is symmetric: `execute_app` is `tokio::spawn`-ed, and the new `Ok(Err(join_err))` arm in the `select!` body routes the synthetic `anyhow::Error` through `Supervisor::handle_app_crash`. Do NOT reintroduce `panic::resume_unwind` for panicking app tasks — that's the bug the worker is now hardened against. Add `Self::handle_app_crash` to the `match` arms if you ever add a new failure class (OOM, store-limit-exceeded, etc.).
...
- **Persisted interfaces** (kv-store, cache, scheduling) honor `EDGE_KV_STORE_PATH` / `EDGE_CACHE_PATH` / `EDGE_SCHEDULING_PATH` env vars. Absent or invalid → ephemeral in-memory only. **Lifecycle:** stop / crash / rebalance keep these dirs in place. Deletion happens via issue #569 — `DELETE /api/v1/admin/apps/{appName}` and `DELETE /api/v1/admin/tenants/{id}` publish a `task_purge` TaskMessage variant to each worker via the outbox (Commit 4/5 of PR for #569); the worker calls `edge_runtime::runtime::purge_tenant` which removes the in-memory registry entry and the on-disk directories. Forward-compatible with #475/#476 (CP-side durable `tenant_kv` Postgres backend) — when those ship, the same outbox call site adds a sibling `tenant_kv BatchDelete` row. **KV / cache / scheduling are per-tenant by design** (issue #558) — two apps of the same tenant share one namespace (apps opt in to sharing; task IDs are UUIDs so scheduling cross-app collision is theoretical). **Filesystem preopens (`EDGE_FS_PATH`) are per-app**: the guest's `/` is mounted at `{EDGE_FS_PATH}/{tenant_id}/{app_name}/`. Per-tenant would let app A corrupt app B's SQLite (miserable to debug), so preopens get a different default.
- **Destructive CLI commands (issue #573, issue #60).** `edge apps delete <name> --yes` is the first owner-role destructive subcommand (the CP's `RequireRole("owner")` middleware gates `DELETE /api/v1/admin/apps/{appName}`; the CLI loads a single bearer via `ApiKey::load` with no separate admin-token path, so a pre-flight `whoami` at `edge-cli/src/commands/apps.rs::check_owner_role` surfaces an actionable role-mismatch message instead of a generic 403). Convention for any future admin-only destructive command (`apps delete`, `tenant delete`, `secret reset`, etc.): (a) pre-flight `whoami` role check + multi-line actionable error on mismatch; (b) `--yes`/`-y` flag is REQUIRED (flat bail — no TTY confirm prompt, since the cascade reaches beyond the CP via `task_purge` outbox rows that tear down worker in-memory + on-disk state); (c) retry-aware via `call_with_retry_no_interrupt` with the default retry constants (DELETE is naturally idempotent — second call returns 404); (d) wrap typed `ApiError` via `.map_err(anyhow::Error::new)` so `is_anyhow_retryable` can downcast. The wire layer is `ApiClient::delete_app` at `edge-cli/src/api/client.rs:1147` (mirrors `Keys::revoke`). The cascade is irreversible (artifact blobs, env rows, active deployments, per-app KV/cache/scheduler dirs on every worker via `task_purge` per issue #569 / PR #601). **5xx retry semantics (issue #60):** a 5xx returned by the cascade (the DB-tx body — parent delete or any of the four child deletes — failing) is logged-and-rolled-back inside the CP and surfaces to the CLI as `ApiError::Transient`, so the CLI retries up to `DEFAULT_MAX_RETRIES + 1` attempts. The CLI's surface message ("apps delete failed after retries") is unchanged. A separate 5xx can fire from the **post-commit artifact-cleanup pass** — the DB rows and outbox enqueue have already committed by then, so the failure leaves the CP consistent but with orphan `.wasm`/`.cwasm` files in storage; the CLI bails the same way and a follow-up `edge apps delete --yes` is a no-op (the parent row is gone, so DELETE…RETURNING returns no rows and the handler returns 404 → CLI prints "not found"). Net result: **no half-committed state** for the caller to reason about.
- **CLI retry classification lives on `ApiError` (issue #571).** `ApiError::is_retryable()` at `edge-cli/src/api/client.rs:57-69` is the single source of truth for "should the caller retry?" — `Transient { .. }` and `Rejected { 429 }` are retryable; every other `Rejected` is deterministic and must NOT retry. `From<serde_json::Error>` for `ApiError` returns `Rejected { status: 200, body: "invalid response body: ..." }` (issue #571 retry-propagation follow-up) so wire-shape mismatches are deterministic — the server returns the same bytes on retry, retrying wastes the budget. The canonical retry loop is `commands::retry::call_with_retry<T, F>(...)` at `edge-cli/src/commands/retry.rs`. Every retry-aware CLI command (`edge deploy`, `edge env delete`, `edge env list`, `edge traffic set`, `edge traffic show`, `edge keys revoke`, `edge keys list`, `edge domains remove`, `edge domains list/check`, `edge egress set/clear/show`) routes through this one loop. Generic over `T`, closure-typed so callers pass borrowed arguments and the same closure shape covers deploy, env, traffic, egress, keys, and domains. `edge traffic set` and `edge env delete` expose `--max-retries` / `--retry-base-ms` / `--retry-cap-ms` (operator-tunable tier, defaults 3 / 500 / 8_000, `--max-retries` clamped to 0..=20 by `value_parser`); the rest use hardcoded defaults from `commands::retry::{DEFAULT_MAX_RETRIES, DEFAULT_RETRY_BASE_MS, DEFAULT_RETRY_CAP_MS}` (single source of truth — no per-module duplication). Non-deploy callers should use the `commands::retry::call_with_retry_no_interrupt` shim — it constructs a fresh `AtomicBool::new(false)` internally so the 11 retry-aware sites don't carry dead-weight `AtomicBool` ceremony; the deploy caller keeps the explicit `&AtomicBool` parameter because it installs its own ctrlc handler. The `EDGE_CLI_RETRY_BASE_MS` env var is a **test-only hook** (not a stable user-facing API) used by the wiremock integration tests to shrink the backoff below the 500ms default; production callers must NOT rely on it. Endpoints that need a server-side `Idempotency-Key` header (`edge env set`, `edge keys create`, `edge domains add`) are NOT routed through `call_with_retry` yet — see the Phase-2 follow-up issue. Wrap the error with `anyhow::Error::new(api_err).context("...")` (NOT plain `anyhow!("...")`) so the typed `ApiError` survives the chain and `is_anyhow_retryable` (the canonical helper at `commands::retry::is_anyhow_retryable` — was `commands::deploy::is_anyhow_retryable` pre-follow-up) can downcast it: walk `e.chain()`, downcast to `ApiError` first (defer to `is_retryable`), then to `reqwest::Error` (`!is_builder()` → transient, `is_builder()` → deterministic). Any error in the chain that isn't one of those is treated as deterministic (don't retry). The deploy-side shim `commands::deploy::deploy_with_retry` is a 5-line wrapper that calls `call_with_retry("deploy", ...)` — the public surface (clap flags, `run_upload` body) is unchanged.
...
- **Per-tenant worker JWT mint has a hosting constraint (issue #491 #2).** `POST /api/internal/tokens/tenant` (defined at `edge-control-plane/internal/handler/internal.go:836`) only issues tokens for tenants currently in `worker_status.apps` with `status='running'` for the calling worker. A freshly-bootstrapped worker that hasn't heartbeated has no `worker_status.apps` entries and must heartbeat before its first mint. The 403 fires regardless of inbound JWT scope — wildcard ("bootstrap") and previously-minted scoped tokens both go through the same hosting check. The route is rate-limited at three layers: per-IP (60/s sustained, 30 burst), per-worker (10/s, 5 burst), and per-`tenant_id`-in-body (30/s, 10 burst); see `edge-control-plane/internal/app/app.go:697-708`.
