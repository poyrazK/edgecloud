//! `globalThis.EdgeCloud` namespace registrars — long-running world.
//!
//! **Mirror of `edge-js-runtime::register`** (the FaaS-world
//! counterpart). The bodies are identical; only the `use` paths
//! differ. This file is the canonical duplication site for issue
//! #426 — see the FaaS-side `edge-js-runtime/src/register.rs` header
//! for the full rationale (wit_bindgen::generate! produces world-bound
//! module trees, so the bodies can't share via a `pub use` chain).
//!
//! The `register_all` entry point is the only public symbol; the
//! per-namespace `register_*` helpers stay private. `wasm_only::run`
//! calls `register::register_all(&ctx)?` exactly once before evaluating
//! the user's bytecode.

use rquickjs::{Ctx, Function, Object, TypedArray, Value};

// Bindgen-generated `edge:cloud/*` import modules from the LR-world
// `wasm_only` in this crate. The parent `wasm_only` does `pub use
// self::edge::cloud::{kv_store, cache, ...};` so this module can
// reach them at `super::wasm_only::*` without naming the
// bindgen-internal `edge::cloud::` path.
