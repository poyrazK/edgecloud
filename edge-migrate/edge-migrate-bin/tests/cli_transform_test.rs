...
fn test_transform_flag_outputs_wasi_header() {
    ...
    assert!(
        stdout.contains("#include <wasi/sockets.h>"),
        "WASI header missing"
    );
    // WASI socket create must be present
    assert!(
        stdout.contains("wasi_socket_tcp_create"),
        "wasi_socket_tcp_create missing"
    );
    // Original socket call must NOT be present (after header lines)
    // Use a longer, unambiguous pattern from the original call
    assert!(
        !stdout.contains("AF_INET, SOCK_STREAM, 0"),
        "original socket(AF_INET, SOCK_STREAM, 0) still present"
    );
}
