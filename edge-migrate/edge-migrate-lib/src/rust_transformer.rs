...
use crate::transformer::{Transformation, TransformError, TransformResult};
...
impl RustTransformer {
    ...
    pub fn transform(&self, source: &str, matches: Vec<PatternMatch>) -> TransformResult {
        ...
        for m in &sorted {
            ...
            transformations_applied.push(Transformation {
                ...
                description: format!(
                    ...
                    wasi_code.trim_start().split_whitespace().next().unwrap_or(pattern_name)
                ),
            });
        }
        ...
        let transformed_source = String::from_utf8(output)
            .expect("Transformed Rust source is not valid UTF-8");
...
mod tests {
    ...
    fn test_transform_fs_open_and_write() {
        ...
        assert!(r.transformed_source.contains("filesystem::write(\"b.txt\", b\"x\")"));
    }
...
";
    ...
    fn test_transform_no_matches_passes_through_with_prelude() {
        ...
        assert!(r.transformed_source.contains("use wasi::socket::tcp::TcpSocket;"));
        // Original content is preserved verbatim.
        assert!(r.transformed_source.contains("fn main() { println!(\"hi\"); }"));
    }
