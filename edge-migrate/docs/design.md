...
2. **Directory / tree** — `edge-migrate --tree <DIR> [--app-name NAME]` walks the directory for `.c`/`.h` files (skipping `build/`, `target/`, `node_modules/`, etc.) and POSTs the whole tree to `POST /api/migrate-tree`. The developer supplies an explicit `--app-name` that must match `^[a-z0-9][a-z0-9_-]{0,62}$`. Without `--app-name`, the dir basename is used.
...
For directory mode, the explicit `--app-name` is required to be a valid `^[a-z0-9][a-z0-9_-]{0,62}$` name (defense-in-depth + DNS-safety for the eventual `*.edgecloud.dev` URL). See §6.1.2 for the full request/response shape.
...
Fields:
  - app_name: <string>             (required; regex ^[a-z0-9][a-z0-9_-]{0,62}$)
...
| App name length | 1–63 | regex `^[a-z0-9][a-z0-9_-]{0,62}$` |
