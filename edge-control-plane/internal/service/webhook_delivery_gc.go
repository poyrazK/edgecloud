...
// webhookRepoForGC is the subset of *repository.WebhookRepository used
// by WebhookDeliveryGCService. Defining it locally keeps tests
// mockable without a live DB.
type webhookRepoForGC interface {
	DeleteOlderThanBatched(ctx context.Context, retention time.Duration, batchSize, maxBatches int) (int64, error)
}

// WebhookDeliveryGCService periodically deletes webhook_deliveries
// rows older than the configured retention. Mirrors LogGCService
// verbatim (issue #574 retention GC trio). Runs as a background
// goroutine for the lifetime of the control-plane process and exits
...
type WebhookDeliveryGCService struct {
	repo webhookRepoForGC
	sink WebhookDeliveryGCSink
}

func NewWebhookDeliveryGCService(repo webhookRepoForGC, sink WebhookDeliveryGCSink) *WebhookDeliveryGCService {
	if sink == nil {
		sink = func(int64, bool) {} // nil-safe no-op for tests
	}
	return &WebhookDeliveryGCService{repo: repo, sink: sink}
}
...
// env vars (WEBHOOK_DELIVERY_GC_INTERVAL, WEBHOOK_DELIVERY_RETENTION)
...
// webhook_deliveries table. The operator sees a clear log line and the
// GC stays disabled until the env vars are fixed and the process
// restarted.
func (s *WebhookDeliveryGCService) Run(ctx context.Context, interval, retention time.Duration) {
	if interval <= 0 || retention <= 0 {
		log.Printf("webhook_delivery_gc: invalid interval=%s retention=%s; refusing to run", interval, retention)
		return
	}
	...
	runOnce := func() {
		...
		if err != nil {
			...
			log.Printf("webhook_delivery_gc: delete failed (retention=%s): %v", retention, err)
			s.sink(0, true) // issue #581 — record the failed tick
			return
		}
		if deleted > 0 {
			log.Printf("webhook_delivery_gc: deleted %d rows older than %s", deleted, retention)
		}
