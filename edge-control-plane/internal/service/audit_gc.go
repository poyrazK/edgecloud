...
// auditRepoForGC is the subset of *repository.AuditRepository used by
// AuditGCService. Defining it locally keeps tests mockable without a live DB.
type auditRepoForGC interface {
	DeleteOlderThanBatched(ctx context.Context, retention time.Duration, batchSize, maxBatches int) (int64, error)
}

// AuditGCService periodically deletes audit_logs rows older than the
// configured retention. Mirrors LogGCService verbatim (issue #574
// retention GC trio). Runs as a background goroutine for the lifetime of
// the control-plane process and exits cleanly when ctx is cancelled.
//
...
type AuditGCService struct {
	repo auditRepoForGC
	sink AuditGCSink
}

func NewAuditGCService(repo auditRepoForGC, sink AuditGCSink) *AuditGCService {
	if sink == nil {
		sink = func(int64, bool) {} // nil-safe no-op for tests
	}
	return &AuditGCService{repo: repo, sink: sink}
}
...
// env vars (AUDIT_GC_INTERVAL, AUDIT_RETENTION) without changing this struct.
//
...
// audit_logs table. The operator sees a clear log line and the GC stays
// disabled until the env vars are fixed and the process restarted.
func (s *AuditGCService) Run(ctx context.Context, interval, retention time.Duration) {
	if interval <= 0 || retention <= 0 {
		log.Printf("audit_gc: invalid interval=%s retention=%s; refusing to run", interval, retention)
		return
	}
	...
	runOnce := func() {
		...
		if err != nil {
			...
			log.Printf("audit_gc: delete failed (retention=%s): %v", retention, err)
			s.sink(0, true) // issue #581 — record the failed tick
			return
		}
		if deleted > 0 {
			log.Printf("audit_gc: deleted %d rows older than %s", deleted, retention)
		}
