...
// mockWebhookDeliveryGCRepo is the mock the WebhookDeliveryGCService
// tests run against. Mirrors mockLogGCRepo (log_gc_test.go:15-52) —
// same shape, different interface.
type mockWebhookDeliveryGCRepo struct {
	mu    sync.Mutex
	calls []time.Duration
	err   error
	delay time.Duration
...
func (m *mockWebhookDeliveryGCRepo) DeleteOlderThanBatched(ctx context.Context, retention time.Duration, batchSize, maxBatches int) (int64, error) {
	if m.delay > 0 {
		select {
...
func (m *mockWebhookDeliveryGCRepo) callCount() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return len(m.calls)
}

func (m *mockWebhookDeliveryGCRepo) lastRetention() (time.Duration, bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(m.calls) == 0 {
		return 0, false
...
// recordingWebhookDeliverySink mirrors recordingLogSink
// (log_gc_test.go:285-335) — same shape, typed to
// WebhookDeliveryGCSink.
type recordingWebhookDeliverySink struct {
	mu    sync.Mutex
	calls []logSinkCall
}

func (r *recordingWebhookDeliverySink) record(rowsDeleted int64, hadError bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.calls = append(r.calls, logSinkCall{rowsDeleted, hadError})
}

func (r *recordingWebhookDeliverySink) callCount() int {
	r.mu.Lock()
	defer r.mu.Unlock()
	return len(r.calls)
}

func makeRecordingWebhookDeliverySink() (WebhookDeliveryGCSink, *recordingWebhookDeliverySink) {
	r := &recordingWebhookDeliverySink{}
	var sink WebhookDeliveryGCSink = r.record
	return sink, r
}

func TestWebhookDeliveryGC_DeletesOldRows(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, nil)
...
func TestWebhookDeliveryGC_RetentionFromConfig(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, nil)
	...
	defer cancel2()
	go svc.Run(ctx2, 10*time.Second, 30*24*time.Hour)
	time.Sleep(20 * time.Millisecond)
	...
	if got2 != 30*24*time.Hour {
		t.Errorf("second retention = %s, want 30d", got2)
	}
}

func TestWebhookDeliveryGC_TickerFiresAtInterval(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, nil)
...
func TestWebhookDeliveryGC_RepoErrorDoesNotStopLoop(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{err: errors.New("simulated DB outage")}
	svc := NewWebhookDeliveryGCService(repo, nil)
...
func TestWebhookDeliveryGC_ZeroIntervalRefusesToRun(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, nil)
...
func TestWebhookDeliveryGC_NegativeRetentionRefusesToRun(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, nil)
...
// tolerate it.
func TestWebhookDeliveryGC_PreemptsOnCancelledContext(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, nil)
...
func TestWebhookDeliveryGC_RecordsMetrics(t *testing.T) {
	sink, rec := makeRecordingWebhookDeliverySink()
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, sink)
...
func TestWebhookDeliveryGC_NilSink_NoPanic(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepo{}
	svc := NewWebhookDeliveryGCService(repo, nil)
...
func (m *mockWebhookDeliveryGCRepoWithCounting) DeleteOlderThanBatched(_ context.Context, _ time.Duration, _, _ int) (int64, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.calls++
	if m.calls == 1 {
...
func (m *mockWebhookDeliveryGCRepoWithCounting) callCount() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.calls
}

// TestWebhookDeliveryGC_RecordsMetrics_TickCountEqualsSweepCount: a
...
func TestWebhookDeliveryGC_RecordsMetrics_TickCountEqualsSweepCount(t *testing.T) {
	repo := &mockWebhookDeliveryGCRepoWithCounting{firstN: 0, zeroN: 0}
	sink, rec := makeRecordingWebhookDeliverySink()
	svc := NewWebhookDeliveryGCService(repo, sink)
