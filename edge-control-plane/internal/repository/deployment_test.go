...
func TestDeploymentRepository_GetByID(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{
		...
		"protocol",
	}).AddRow("d_1", "t_1", "hello", domain.StatusDeployed, "abc", pq.StringArray{"fra"}, now, true, "", "", []byte{}, 0, nil, nil, nil, "http")

	mock.ExpectQuery(`SELECT d\.id.*FROM deployments d LEFT JOIN apps.*WHERE d\.id = \$1`).
		WithArgs("d_1").
		WillReturnRows(rows)
...
func TestDeploymentRepository_ListByApp(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{
		...
		"signature", "signing_key_id", "build_attestation", "desired_replicas", "preview_id", "preview_pr_number", "preview_expires_at",
		"protocol",
	}).AddRow("d_1", "t_1", "hello", "deployed", "hash1", pq.StringArray{"fra"}, time.Now(), false, "", "", []byte{}, 0, nil, nil, nil, "http").
		AddRow("d_2", "t_1", "hello", "active", "hash2", pq.StringArray{"sfo"}, time.Now(), false, "", "", []byte{}, 0, nil, nil, nil, "tcp")

	mock.ExpectQuery(`SELECT d\.id.*FROM deployments d LEFT JOIN apps.*WHERE d\.tenant_id = \$1 AND d\.app_name = \$2.*ORDER BY d\.created_at DESC`).
		WithArgs("t_1", "hello").
		WillReturnRows(rows)
...
func TestDeploymentRepository_ListByAppPaginated(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT d\.id.*FROM deployments d LEFT JOIN apps.*WHERE d\.tenant_id = \$1 AND d\.app_name = \$2.*ORDER BY d\.created_at DESC LIMIT \$3 OFFSET \$4`).
